% Script to minimize error on boundary (unit square)
%   Brute force

% Source position
x_source = 0.7;
y_source = 0.65;
t = 0.1; % Magically 0.1 works best
RES = 3* 6;
% Search space
x0_vector = linspace(-1, 2, RES);
% y0_vector = linspace(-1, 2, 10);
y0_vector = linspace(0, 1, RES/3);
    ...
    for j = 1:length(y0_vector)
        for k = 1:length(x0_vector)
            for l = 1:length(y0_vector)
                ...
                % If in box
                if x0(1) > 0 && x0(1) < 1 && y0(1) > 0 && y0(1) < 1 || ...
                    x0(2) > 0 && x0(2) < 1 && y0(2) > 0 && y0(2) < 1
                    e_L2_matrix(i,j) = nan;
                % Overlapping sources
                elseif x0(1) == x0(2) && y0(1) == y0(2)
                    ...
                    e_L2_matrix(i,j,k,l) = error_on_boundary(fxy);
                end
            end
        end
    end
