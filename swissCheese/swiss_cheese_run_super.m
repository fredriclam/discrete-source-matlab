...
%   Inputs
%     geometry: string 'cyl' (or 'cylinder') or 'slab'
%     D: sphere diameter
%     xi: key dimension of the domain
%     width: width of the domain (periodic if slab)
%     length: length of the domain
%     density: sphere centres per unit volume
%     plot_flag: whether or not plots should be generated (default: false)
%   Outputs
...
function [propagation_success, time_of_travel, coords, shortest_path] = ...
    swiss_cheese_run_super(geom, D, xi, width, length, density, plot_flag)
...
map_to_cartesian = @(diam1, diam2, length, data) ...
    [ diam1/2 * sqrt(data(:,1)) .* [cos(2*pi*data(:,2))], ...
    diam2/2 * sqrt(data(:,1)) .* [sin(2*pi*data(:,2))], ...
    length * data(:,3) ];
generate_coords_cyl = @(diam1, diam2, length, N) ...
    map_to_cartesian(diam1, diam2, length, rand(N,3));
% Distance function
...
if strcmpi(geom,'cyl') || strcmpi(geom,'cylinder')
    % Choose the coordinate generating function
    generate_coords = generate_coords_cyl;
    % Generalize the distance function
    dist = @(p1, p2, period) dist(p1,p2);
    % Number of particles -- specified via volume * (density == 1)
    N = floor(density*pi/4*xi^2*length);
elseif strcmpi(geom,'slab')
    % Choose the coordinate generating function
    generate_coords = generate_coords_slab;
    % Choose the periodic distance function
    dist = dist_periodic_y;
    % Number of particles -- specified via volume * (density == 1)
...
coords = generate_coords(xi, width, length, N);
% Sort by z (propagation coordinate)
coords = sortrows(coords,3);

% If at most one sphere is generated, return failure
% if size(coords,1) < 2
...
i_fast = 1;
while i_fast <= N && coords(i_fast,3) < length_initiation
...
i = N;
while i >= 1 && i <= N && coords(i,3) > length_success
    % Represent edge in adjacency matrix
    adj_mat(i+1,N+2) = 1;
    adj_mat(N+2,i+1) = 1;
    % Decrement counter
