...
function [diam_equivalent] = ...
    convert_discrete_model_to_hard_sphere(tauc, theta_ign)
...
% end
diam_equivalent = zero_bisection(@(r) max_at_r(tauc,r) - theta_ign);
...
function [G, t_crit] = max_at_r(tauc, r, TOL)
if nargin == 2
    TOL = 1e-9;
elseif nargin ~= 3
...
while heatResponse_1(tauc, [r 0 0], [0 0 0], b, 0) > 0
    step = step * 2;
    b = b + step;
end
% Bisection search
while b-a > tol
    m = 0.5*(a+b);
    if heatResponse_1(tauc, [r 0 0], [0 0 0], m, 0) > 0
        a = m;
    else
        b = m;
    end
